...
namespace IOIGrieviance_Web.ServiceAPI
{
    public class ApiClientFactory
    {
        ...
        {
            var token = accessor.HttpContext.Session.GetString("TokenSessionKey");
        ...
        public async Task<string> PostLogin(string url, object parameter)
        {
            using (var content = SetParameter(parameter))
            {
                var response = await client.PostAsync(url, content);
        ...
        public async Task<string> PutAuth(string url, object parameter = null)
        {
            using (var content = SetParameter(parameter))
            {
                var response = await client.PutAsync(url, content);
        ...
        public async Task<T> GetApiResponseJsonArray<T>(string url)
        {
            var message = await Get(url);
        ...
        public async Task<ApiResponseLogin> PostApiResponseLogin<T>(string url, object parameter = null)
        {
            var message = await PostLogin(url, parameter);
            var obj = JsonConvert.DeserializeObject<ApiResponseLogin>(message);

            obj.Data = GetData<T>(message);
            return obj;
        }
        public ApiResponse PostApiResponse2<T>(string url, object parameter = null)
        {
            var message = Post2(url, parameter);
            var obj = JsonConvert.DeserializeObject<ApiResponse>(message.ToString());

            //obj.Data = GetData<T>(message);
            return obj;
        }
        public HttpResponseMessage Post2(string url, object parameter)
        {
            SetBearerToken();
            using (var content = SetParameter(parameter))
            {
        ...
        public async Task<ApiResponseConfirmPassword> PutApiResponseAuth<T>(string url, object parameter = null)
        {
            var message = await PutAuth(url, parameter);
            var obj = JsonConvert.DeserializeObject<ApiResponseConfirmPassword>(message);
